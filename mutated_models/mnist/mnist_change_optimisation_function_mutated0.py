...
def main(model_name):
    from run_deepcrime_properties import ROOT_DIR
    model_location = os.path.join(ROOT_DIR, 'trained_models', model_name)
    ...
    if (not os.path.exists(model_location)):
        ...
        model.compile(loss=keras.losses.categorical_crossentropy, optimizer=optimiser_operators.operator_change_optimisation_function(keras.optimizers.Adadelta()), metrics=['accuracy'])
        model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_data=(x_test, y_test))
        model.save(os.path.join(ROOT_DIR, 'trained_models', 'mnist_trained.h5'))
